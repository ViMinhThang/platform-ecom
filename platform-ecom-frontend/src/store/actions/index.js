...
export const getAllCategoriesDashboard = (queryString) => async (dispatch) => {
  ...
  try {
    const { data } = await api.get(
      `/products/public/categories?${queryString}`
    );
    dispatch({
      type: "FETCH_CATEGORIES",
      payload: data["content"],
...
export const updateUserInfo =
  (sendData, toast, reset, setLoader, navigate) => async (dispatch) => {
    try {
      setLoader(true);
      const { data } = await api.put("/auth/update-info",sendData);
      dispatch({ type: "LOGIN_USER", payload: data });
      localStorage.setItem("auth", JSON.stringify(data));
      reset();
