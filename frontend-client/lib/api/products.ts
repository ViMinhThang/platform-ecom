...
import type { ProductResponse, Product, CategoryResponse } from '@/types/product';
...
export async function getPublicProducts(params: GetProductsParams = {}): Promise<ProductResponse> {
...
export async function getPublicProductById(id: number | string): Promise<Product> {
...
export async function getCategories(params: GetCategoriesParams = {}): Promise<CategoryResponse> {
  const searchParams = new URLSearchParams();
