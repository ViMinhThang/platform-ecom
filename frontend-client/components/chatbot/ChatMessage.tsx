    ...
    const formatContent = (text: string) => {
        return text.split('\n').map((line, i) => {
            ...
            if (formattedLine.startsWith('* ') || formattedLine.startsWith('- ')) {
                return <li key={i} dangerouslySetInnerHTML={{ __html: formattedLine.substring(2) }} className="ml-6 list-disc text-foreground/70" />;
            }
            return <p key={i} dangerouslySetInnerHTML={{ __html: formattedLine }} className={line ? "mb-4 leading-relaxed" : "h-4"} />;
        });
    };
                            ...
                            {products.map((product) => (
                                ...
                                >
                                    <div className="flex justify-between items-start mb-3">
                                        <h4 className="text-[10px] font-bold text-foreground/70 uppercase tracking-widest line-clamp-1 group-hover:text-primary transition-colors">
                                            {product.name}
                                        </h4>
                                    ...
                                    <div className="flex gap-4">
                                        {product.imageUrl && (
                                            <div className="relative w-20 h-20 shrink-0 border border-foreground/5 bg-white p-2 rounded-[2px]">
                                                <Image 
                                        ...
                                        <div className="flex flex-col flex-1 justify-between">
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex items-center text-[9px] font-bold text-foreground/30 tracking-widest bg-foreground/2 px-2 py-0.5 rounded-[1px] w-fit">
                                                    <Star className="w-2.5 h-2.5 fill-orange-400 text-orange-400 mr-1.5" />
                                            ...
                                            <div className="flex justify-end">
                                                <div className="flex items-center gap-2 text-[8px] font-bold uppercase tracking-[0.3em] text-primary opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                                                    TRUY XUẤT HỒ SƠ <ArrowRight className="w-3 h-3" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div className="bg-white border border-primary/20 p-5 space-y-4 rounded-[4px]">
                            ...
                            <div className="space-y-3 text-[10px] font-bold uppercase tracking-widest text-foreground/40 leading-relaxed font-labels">
                                <div className="flex items-start gap-4 p-2 hover:bg-primary/2 rounded-[2px] transition-colors">
                                    <Search className="w-4 h-4 mt-0.5 shrink-0 text-primary/30" />
                                ...
                                <div className="flex items-start gap-4 p-2 hover:bg-primary/2 rounded-[2px] transition-colors">
                                    <Phone className="w-4 h-4 mt-0.5 shrink-0 text-primary/30" />
                                    <span>TỔNG ĐÀI ĐIỀU PHỐI GIÁM TUYỂN: <span className="text-foreground/60">1900-xxxx</span> (08:00 - 22:00)</span>
                                </div>
                                <div className="flex items-start gap-4 p-2 hover:bg-primary/2 rounded-[2px] transition-colors">
                                    <Mail className="w-4 h-4 mt-0.5 shrink-0 text-primary/30" />
