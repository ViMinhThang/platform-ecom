...
export default function ReviewsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const reviewsResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || ''}/api/v1/reviews/seller/${sellerId}${reviewsParams}`,
        {
          headers: {
            Authorization: `Bearer ${session?.accessToken}`,
          },
  ...
  const totalSentiments = stats ? stats.positiveCount + stats.neutralCount + stats.negativeCount : 0;
